...
// HECTOR: Clean this shit up into a decent options object
const width = 1200;
const height = 800;
const minWidth = 1000;
const minHeight = 700;
 ...
 */
function createDirSync(p) { // HECTOR: unnecessary, mkdir already checks for existing files
	if (!fs.existsSync(p)) {
		fs.mkdirSync(p);
	}
}
...
function startNode(options, mongoChild, callback) {
	...
	const rootUrl = `${domain}:${options.webPort}/`; // ideally this should just be domain
	let opened = false;

	const nodeEnv = {
		ROOT_URL: rootUrl, // BUG: passing the PORT twice? Test this on production version
		PORT: options.webPort, // here you pass it the second time
		DB_PATH: dataPath,
...
// BUG: change this for window-all-closed ? This is an OSX-specific behaviour
...
app.on('activate-with-no-open-windows', () => {
	if (global.mainWindow) {
		global.mainWindow.show(); // BUG: does this even exist if there are no open windows?
	}
